...
export default function CreatureBlock({
  ...
  const creatureHeader = (
    <div className="text-xl pb-4 italic duration-700 ease-in-out animate-in fade-in slide-in-from-bottom-2 mt-3">
      <h1 className="text-2xl font-semibold border-b-2 mb-1">Appearance</h1>
      <p className='mb-3'>{appearance}</p>
      <h1 className="text-2xl font-semibold border-b-2 mb-1">Lore</h1>
      <p>{lore}</p>
    </div>
  );
...
const reactionActionSection = (creature: creatureSchemaType) => {
  ...
  const actionMap = reactions.map((action) => {
...
const legendaryActionSelection = (creature: creatureSchemaType) => {
  ...
  const actionMap = legendary.actions.map((action) => {
...
const actionSection = (
  ...
  if (targetedWeaponAttacks) {
    const targetedWeaponAttacksUI = targetedWeaponAttacks.map((action) => {
      const {
        name,
        attackStat,
        attackType,
  ...
  if (savingThrowAttacks) {
    const saveAttacksUI = savingThrowAttacks.map((action) => {
  ...
  if (specialActions) {
    const specialActionsUI = specialActions.map((action) => {
      const { name, description } = action;
      return (
        <div key={name} className="mb-4">
          <p>
...
function StatsBlock({ stats }: { stats: statsType }) {
  const statBlocks = orderedKeys.map((stat: keyof statsType, i) => {
