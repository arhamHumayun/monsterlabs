...
`;
export async function createCreature(prompt: string) : Promise<{ data?: creatureSchemaType, error?: string }> {

  const { data, error } = await routeLogicGPT(prompt, 0);
...
async function routeLogicGPT(prompt: string, attempts: number = 0): Promise<{ data?: creatureSchemaType, error?: string }> {
